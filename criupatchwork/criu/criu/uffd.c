...
static int prepare_sock_addr(struct sockaddr_un *saddr)
{
	...
	len = snprintf(saddr->sun_path, sizeof(saddr->sun_path),
        ...
        printf("saddr->sun_path= %s, ",saddr->sun_path);
...
static struct lazy_pages_info *ud_open(int client)
{
	...
	ret = recv(client, &lpi->pid, sizeof(lpi->pid), 0);
        printf("## ret=%d, sizeof(lpi->pid)=%ld, from recv() 1\n",ret, sizeof(lpi->pid));
...
int cr_lazy_pages()
{
	...
	int epollfd;
	int ret;
        printf("uffd.c check_for_uffd 1\n");
	if (check_for_uffd())
		return -1;
        printf("uffd.c lpi_hash_init 2\n");
	lpi_hash_init();
printf("uffd.c lazy_pages_prepare_pstree 3\n");
	if (lazy_pages_prepare_pstree())
		return -1;
printf("uffd.c prepare_epoll 4\n");
	epollfd = prepare_epoll(task_entries->nr_tasks, &events);
	if (epollfd < 0)
		return -1;
printf("uffd.c prepare_uffds 5\n");
	if (prepare_uffds(epollfd))
		return -1;
printf("uffd.c connect_to_page_server 6\n");
	if (connect_to_page_server())
		return -1;
printf("uffd.c handle_requests 7\n");
