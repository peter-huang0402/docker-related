...
static int setup_opts_from_req(int sk, CriuOpts *req)
{
	...
	// log_set_loglevel(req->log_level);  // peter added!! cause error!!
log_set_loglevel(DEFAULT_LOGLEVEL);   // peter added!!
	...
	if (req->has_file_locks)
        ...
        // peter for lazy-pages
        if (req->lazy_pages){
           printf("## enable  opts.lazy_pages = req->lazy_pages=%d, ## \n",req->lazy_pages );
           opts.lazy_pages = req->lazy_pages;
        } 

        if (req->lazy_pages_port){
           opts.port = htons((short)req->lazy_pages_port);
           printf("## enable  opts.port=%d, req->lazy_pages_port=%d, ## \n",opts.port, req->lazy_pages_port);
        } 
...
static int restore_using_req(int sk, CriuOpts *req)
{
	...
	//opts.lazy_pages=true; 
        printf("peter add, opts.lazy_pages=%d,\n",opts.lazy_pages);
