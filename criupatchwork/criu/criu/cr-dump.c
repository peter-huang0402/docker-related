...
{ 
	...
	if (collect_namespaces(false) < 0)
        ...
        printf("## opts.port=%d, opts.use_page_server=%d, opts.addr=%s,\n",opts.port,opts.use_page_server, opts.addr);
...
int cr_dump_tasks(pid_t pid)
{
 ...
 printf("## opts.port=%d, opts.use_page_server=%d, opts.addr=%s,\n",opts.port,opts.use_page_server, opts.addr);
	...
	for_each_pstree_item(item) {
        ...
        printf("## cr-dump ->cr_dump_tasks() 0.\n");
	/*
	...
	if (dead_pid_conflict())
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 1.\n");
	/* MNT namespaces are dumped after files to save remapped links */
	if (dump_mnt_namespaces() < 0)
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 2.\n");
	if (dump_file_locks())
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 3.\n");
	if (dump_verify_tty_sids())
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 4.\n");
	if (dump_zombies())
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 5.\n");
	if (dump_pstree(root_item))
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 6.\n");
	if (root_ns_mask)
		if (dump_namespaces(root_item, root_ns_mask) < 0)
			goto err;
 printf("## cr-dump ->cr_dump_tasks() 7.\n");
	ret = dump_cgroups();
	if (ret)
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 8.\n");
	ret = cr_dump_shmem();
	if (ret)
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 9.\n");
	ret = fix_external_unix_sockets();
	if (ret)
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 10.\n");
	ret = tty_post_actions();
	if (ret)
		goto err;
 printf("## cr-dump ->cr_dump_tasks() 11.\n");
