...
static int write_pages_loc(struct page_xfer *xfer,
		int p, unsigned long len)
{       printf("## wrtie_pages = write_pages_loc() \n"); 
	ssize_t ret;
...
{       printf("##write_pagemap == write_pagemap_loc \n");
	int ret;
...
{       printf("## page-xfer.c -> page_server_get_pages() \n");
...
{       printf("## page-xfer.c -> page_server_serve() ;");
	int ret = -1;
	bool flushed = false;
	...
	while (1) {
		...
		ret = recv(sk, &pi, sizeof(pi), MSG_WAITALL);
                // printf("## page-xfer.c -> page_server_serve() --> recving ;  ");
		if (!ret)
			break;
...
printf("## recving PS_IOV_LAZY ; ");
		...
		{
			int32_t status = 0;
printf("## recving PS_IOV_FLUSH_N_CLOSE ; ");
			ret = 0;
...
{       printf("page-xfer.c -> connect_to_page_server() -> 1 \n");  
	if (!opts.use_page_server)
		return 0;
        printf("page-xfer.c -> connect_to_page_server() -> 2 \n"); 
	if (opts.ps_socket != -1) {
		page_server_sk = opts.ps_socket;
		pr_info("Re-using ps socket %d\n", page_server_sk);
		goto out;
	}
        printf("page-xfer.c -> connect_to_page_server() -> 3 \n"); 
