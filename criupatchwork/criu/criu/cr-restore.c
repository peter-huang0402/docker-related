...
printf("## clone(restore_task_with_children \n");
	/*
...
{       printf("## restore_task_with_children \n");
	...
	pid = getpid();
        printf("## Pid=%d, current->pid.virt=%d, \n",pid,current->pid.virt);
...
{
	int ret = -1;
        printf("cr_plugin_init 1 \n");
	if (cr_plugin_init(CR_PLUGIN_STAGE__RESTORE))
		return -1;
 printf("check_img_inventory 2 \n");
	if (check_img_inventory() < 0)
		goto err;
 printf("init_stats 3 \n");
	if (init_stats(RESTORE_STATS))
		goto err;
 printf("kerndat_init_rst 4 \n");
	if (kerndat_init_rst())
		goto err;
 printf("timing_start 5 \n");
 ...
 printf("prepare_task_entries 8 \n");
	if (prepare_task_entries() < 0)
		goto err;
 printf("prepare_pstree 9 \n");
	if (prepare_pstree() < 0)
		goto err;
 printf("crtools_prepare_shared 10 \n");
	if (crtools_prepare_shared() < 0)
		goto err;
 printf("criu_signals_setup 11 \n");
	if (criu_signals_setup() < 0)
		goto err;
 printf("prepare_lazy_pages_socket 12 \n");
	if (prepare_lazy_pages_socket() < 0)
		goto err;
 printf("restore_root_task 13 \n");
