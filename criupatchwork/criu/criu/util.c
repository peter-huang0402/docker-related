...
inline int open_pid_proc(pid_t pid)
{
	...
	dfd = get_service_fd(PROC_FD_OFF);
        printf("open_pid_proc dfd=%d, ",dfd);
	if (dfd < 0) {
...
{       printf("## util.c -> setup_tcp_server() \n");
	int sk = -1;
	struct sockaddr_in saddr;
	socklen_t slen = sizeof(saddr);

	pr_info("Starting %s server on port %u\n", type, (int)ntohs(opts.port));
        printf("###Starting %s server on port=%u, opts.addr=%s\n", type, (int)ntohs(opts.port), opts.addr);
	...
	if (opts.port == 0) {
		if (getsockname(sk, (struct sockaddr *)&saddr, &slen)) {
			...
			goto out;
                ...
                printf("### getsockname, saddr.sin_addr=%d\n",saddr.sin_addr.s_addr);
...
{       printf("## util.c -> run_tcp_server() sk=%d,\n",sk);
	int ret;
	...
	if (sk >= 0) {
		...
		if (*ask < 0){
                        printf("Can't accept connection to server");
...
{       printf("## util.c -> setup_tcp_client() \n");
	struct sockaddr_in saddr;
	int sk;
	...
	if (connect(sk, (struct sockaddr *)&saddr, sizeof(saddr)) < 0) {
		...
		return -1;
            ...
            printf("##connection to server already. \n");
        }
